...
export default function AdminDashboard({ bookings, currentUser, offices = [] }: Omit<AdminDashboardProps, 'role' | 'setView'>) {
    ...
    const calendarData = useMemo(() => {
        ...
        for (let i = 1; i <= lastDate; i++) {
            ...
            const monthDayKey = `${String(month + 1).padStart(2, '0')}-${String(i).padStart(2, '0')}`;
            const isToday = today.getDate() === i && today.getMonth() === month && today.getFullYear() === year;
            const dayBookings = filteredBookings.filter(b => b.date === dateKey);
            
            const currentDayOfWeek = new Date(year, month, i).getDay();
            const isSunday = currentDayOfWeek === 0;
            const holidayName = holidays[monthDayKey] || null;
        ...
        <div className="p-8 bg-slate-50 dark:bg-slate-900 min-h-full transition-colors duration-300">
            <div className="max-w-6xl mx-auto space-y-10">
                ...
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-2">Regional Hub</h2>
                        <div className="flex items-center gap-3">
                            <span className="flex items-center text-[11px] font-black uppercase tracking-[0.2em] text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/40 px-3 py-1.5 rounded-lg border border-blue-100 dark:border-blue-800/50">
                                <i className="fas fa-map-marker-alt mr-2"></i> {currentUser?.assignedRegion || 'All Regions'} Operations
                            </span>
                            ...
                            <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                                <div className="flex flex-col">
                                    <h3 className="text-lg font-black text-slate-800 dark:text-white flex items-center uppercase tracking-tighter">
                                        <i className="fas fa-calendar-alt mr-3 text-blue-500"></i> Availability Calendar
                                    </h3>
                                </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-xl group">
                            <h4 className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mb-6 flex items-center">
                                <i className="fas fa-info-circle mr-2 text-blue-500"></i> Regional Scope
                            </h4>
                            <div className="space-y-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{currentUser?.assignedRegion || 'All Regions'}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Assigned Office</p>
                                        <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{currentUser?.assignedOffice || 'All Offices'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="bg-gradient-to-br from-slate-900 to-black p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
                            <i className="fas fa-shield-alt text-6xl absolute -right-4 -bottom-4 opacity-10 group-hover:scale-110 transition-transform"></i>
                            <h4 className="font-black text-lg mb-2 uppercase tracking-tighter tracking-tighter">System Log</h4>
                            <p className="text-xs text-slate-400 leading-relaxed font-bold italic">
                                Monitoring activity across regional facilities.
                                <br/> <span className="text-blue-400">System Online.</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-8 rounded-[2.5rem] text-white shadow-2xl shadow-indigo-200 dark:shadow-none relative overflow-hidden group">
                        <i className="fas fa-building text-9xl absolute -right-6 -bottom-6 opacity-10 group-hover:scale-110 transition-transform duration-500"></i>
                        <div className="relative z-10">
                            <p className="text-indigo-100 text-xs font-black uppercase tracking-widest mb-1">Total Requests</p>
                            ...
                            <div className="inline-flex items-center px-3 py-1 bg-white/20 rounded-full text-[10px] font-bold backdrop-blur-sm">
                                <i className="fas fa-chart-bar mr-1.5"></i> Regional Activity
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <p className="text-slate-400 dark:text-slate-500 text-xs font-black uppercase tracking-widest mb-1">To be Reviewed</p>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <p className="text-slate-400 dark:text-slate-500 text-xs font-black uppercase tracking-widest mb-1">Confirmed Bookings</p>
                            ...
                            <div className="inline-flex items-center px-3 py-1 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 rounded-full text-[10px] font-bold">
                                <i className="fas fa-check mr-1.5"></i> Area Ready
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 dark:bg-black/80 backdrop-blur-md p-4">
                        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in duration-300 border border-slate-100 dark:border-slate-700">
                            <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
                                <h3 className="font-black text-slate-900 dark:text-white text-xl tracking-tight">Schedule: {selectedDateBookings.date}</h3>
                            ...
                            <div className="p-8 max-h-[60vh] overflow-y-auto space-y-6">
                                {selectedDateBookings.bookings.length > 0 ? (
                                    ...
                                    <div className="text-center py-16 flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-slate-400 dark:text-slate-500 text-sm italic font-bold">No bookings scheduled for this date in your area.</p>
                                    </div>
                                )}
                            </div>
                            <div className="p-6 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex justify-end">
                                <button onClick={() => setSelectedDateBookings(null)} className="w-full py-4 bg-slate-900 dark:bg-blue-600 text-white font-black text-sm rounded-2xl hover:opacity-90 transition-all shadow-xl active:scale-95 uppercase tracking-widest">Close Area View</button>
                            </div>
                        </div>
                    </div>
                )}
